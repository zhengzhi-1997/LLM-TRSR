...
def main():
    ...
    df_test = pd.read_parquet(os.path.join(data_path, 'test.parquent'))
    test_dataset = Dataset.from_pandas(df_test)
    test_dataset = test_dataset.map(encode, batched=False, num_proc=16)
    ...
    best_folder, best_result = None, 0
    for folder in checkpoint_folders:
        if os.path.exists(os.path.join(output_dir, f'val/{folder}.json')):
            with open(os.path.join(output_dir, f'val/{folder}.json'), 'r') as f:
                results = json.load(f)
                if results['recall@3'] + results['recall@5']+ results['recall@10'] > best_result:
                    best_folder, best_result = folder, results['recall@3'] + results['recall@5']+ results['recall@10']
    
    print(f'test: {best_folder}')
    # model = AutoModelForCausalLM.from_pretrained(os.path.join(output_dir, best_folder)).to("cuda:{}".format(args.device))
    # if args.model_dir == 'public/EleutherAI/pythia-160m':
    #     model = GPTNeoXForCausalLM.from_pretrained(os.path.join(output_dir, folder)).to("cuda:{}".format(args.device))
    # elif args.model_dir == 'public/bigscience/bloom-560m':
    #     model = BloomForCausalLM.from_pretrained(os.path.join(output_dir, folder)).to("cuda:{}".format(args.device))
    # elif args.model_dir == 'public/opt/opt-350m':
    #     model = OPTForCausalLM.from_pretrained(os.path.join(output_dir, folder)).to("cuda:{}".format(args.device))
    # else:
    #     model = AutoModelForCausalLM.from_pretrained(os.path.join(output_dir, folder)).to("cuda:{}".format(args.device))

    if args.lora == 'no':
        if args.model_dir == 'public/EleutherAI/pythia-160m':
            model = GPTNeoXForCausalLM.from_pretrained(os.path.join(output_dir, best_folder), torch_dtype=torch.bfloat16).to("cuda:{}".format(args.device))
        elif args.model_dir == 'public/bigscience/bloom-560m':
            model = BloomForCausalLM.from_pretrained(os.path.join(output_dir, best_folder), torch_dtype=torch.bfloat16).to("cuda:{}".format(args.device))
        elif args.model_dir == 'public/opt/opt-350m':
            model = OPTForCausalLM.from_pretrained(os.path.join(output_dir, best_folder), torch_dtype=torch.bfloat16).to("cuda:{}".format(args.device))
        else:
            model = AutoModelForCausalLM.from_pretrained(os.path.join(output_dir, best_folder), torch_dtype=torch.bfloat16).to("cuda:{}".format(args.device))
    elif args.lora == 'yes':
        model = AutoModelForCausalLM.from_pretrained(args.model_dir, torch_dtype=torch.bfloat16)
        model = PeftModel.from_pretrained(model, os.path.join(output_dir, best_folder)).to("cuda:{}".format(args.device))

    results = get_results(tokenizer, model, test_dataloader, args)

    if not os.path.exists(os.path.join(output_dir, 'test')):
        os.makedirs(os.path.join(output_dir, 'test'))
    # # 如果有老的test数据，清除之
    # else:
    #     files = [f for f in os.listdir(os.path.join(output_dir, 'test')) if os.path.isfile(os.path.join(os.path.join(output_dir, 'test'), f))]
    #     for file in files:
    #         file_path = os.path.join(os.path.join(output_dir, 'test'), file)
    #         os.remove(file_path)
    ...
    with open(os.path.join(output_dir, f'test/{best_folder}.json'), 'w') as f:
        # json.dump(results, f)
